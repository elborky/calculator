...
- **Focus states (keyboard input is in scope, M2):** every interactive button shows a clearly visible focus ring — `2px solid var(--accent)` with `2px` offset — never `outline: none` without a replacement. Focus order follows reading order across the keypad. The accent ring is chosen to be visible on both glass themes.
